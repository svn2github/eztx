...
namespace System.Windows.Forms
{
	...
	public class CustomTabControl : TabControl
	{
        ...
        [Description("自定义TabControl属性"), Category("Appearance"), DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
...
//		}
		...
		private void DrawTabPage(int index, Graphics graphics)
		{
			...
			using (GraphicsPath tabPageBorderPath = this.GetTabPageBorder(index)) {
                ...
                using (Brush fillBrush = this._StyleProvider.GetPageBackgroundBrush(index))//new SolidBrush(Color.Red)
                {
		...
		private void DrawTabBorder(GraphicsPath path, int index, Graphics graphics)
		{
			...
			Color borderColor;
			if (index == this.SelectedIndex) {
                borderColor = this._StyleProvider.BorderColorSelected;//this.TabPages[index].BackColor;
		...
		private void DrawTabText(int index, Graphics graphics)
		{
			...
			if (this.SelectedIndex == index) {
				using (Brush textBrush = new SolidBrush(this._StyleProvider.TextColorSelected)){
                    graphics.DrawString(this.TabPages[index].Text, this._StyleProvider.TabFontSelect, textBrush, tabBounds, this.GetStringFormat());
				}
			} else {
				if (this.TabPages[index].Enabled) {
					using (Brush textBrush = new SolidBrush(this._StyleProvider.TextColor)){
                        graphics.DrawString(this.TabPages[index].Text, this._StyleProvider.TabFont, textBrush, tabBounds, this.GetStringFormat());
					}
				} else {
