...
namespace _3d
{
    public partial class AdminCtrlera : Form
    {
        ...
        private void AdminCtrlera_FormClosed(object sender, FormClosedEventArgs e)
        {
            this.Dispose();
        }
        ...
        private void dgvGetInfo()
        {
            ...
            string us = Global.user_province.Substring(us1, us2 - us1);
            ...
            try
            {
                ...
                if (prov.Equals("显示全部"))
                {
                    DataSet ds = lms.conn("select user_name as '用户名',user_realname as '姓名',user_phone as '手机/电话',user_qq as 'QQ/MSN',CASE allowlogin when '1' then'是' ELSE'否' end AS '允许登录',case online when '1' then '在线' else '离线' end as '当前是否在线' from user_login where user_province like '%" + us + "%' and user_vali!='1' and user_vali!='3' and user_vali!='4'");
                    DataTable tb = ds.Tables[0];

                    dataGridView1.DataSource = ds;
                    dataGridView1.DataMember = ds.Tables[0].TableName;

                    DataSet countUser = lms.conn("SELECT count(user_id) as 总计 FROM user_login where user_province like '%" + us + "%' and user_vali!='1' and user_vali!='3' and user_vali!='4'");
                ...
                if (prov.Equals("在线状态"))
                {
                    ...
                    DataSet ds = lms.conn("select user_name as '用户名',user_realname as '姓名',user_phone as '手机/电话',user_qq as 'QQ/MSN',CASE allowlogin when '1' then'是' ELSE'否' end AS '允许登录',case online when '1' then '在线' else '离线' end as '当前是否在线' from user_login where online='" + ol + "' and user_province like '%" + us + "%' and user_vali!='1' and user_vali!='3' and user_vali!='4'");
                    DataTable tb = ds.Tables[0];

                    dataGridView1.DataSource = ds;
                    dataGridView1.DataMember = ds.Tables[0].TableName;

                    DataSet countUser = lms.conn("SELECT count(user_id) as 总计 FROM user_login where online='" + ol + "' and user_province like '%" + us + "%' and user_vali!='1' and user_vali!='3' and user_vali!='4'");
                    DataTable cu = countUser.Tables[0];
                    DataRow dr = cu.Rows[0];
                    this.sumUserLabel.Text = "共有 " + dr[0].ToString() + " 位用户";
                }
        ...
        private void modifyAllowLogin()
        {
            ...
            try
            {
                lms.conn("update user_login set allowlogin='" + radioValue + "' where user_name='" + user_name + "'");
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                lms.conn("update user_login set online='0' where user_name='" + user_name + "'");
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.OK)
            {
                try
                {
                    DataSet ds = lms.conn("delete from user_login where user_name='" + user_name + "'");
        ...
        private void AdminCtrlera_ResizeBegin(object sender, EventArgs e)
        {
            LockWindowUpdate(this.Handle);
        }

        private void AdminCtrlera_ResizeEnd(object sender, EventArgs e)
        {
