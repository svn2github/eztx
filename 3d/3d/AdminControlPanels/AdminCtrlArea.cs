...
namespace _3d
{
    public partial class AdminCtrlArea : Form
    {


        public AdminCtrlArea()
        {
            InitializeComponent();
        }

        private void AdminCtrlArea_Load(object sender, EventArgs e)
        {
        ...
        private void AdminCtrlArea_FormClosed(object sender, FormClosedEventArgs e)
        {
            this.Dispose();
        }
        ...
        private void dgvGetInfo()
        {
            try
            {
                ...
                if (prov.Equals("显示全部"))
                {
                    dgvGetInfoSqlExecute("where parent_name='" + Global.user_name + "' and user_vali='2'");
                }
                if (prov.Equals("在线状态"))
                {
                    if (ol.Equals("在线"))
                    ...
                    dgvGetInfoSqlExecute("where " + ol + " and parent_name='" + Global.user_name + "' and user_vali='2'");
                }
            }
            catch
            {
        ...
        private void searchUser()
        {
            ...
            string where = "where (user_realname LIKE '%" + searchTag + "%' or user_name LIKE '%" + searchTag + "%')  and isdel='1' and parent_name='" + Global.user_name + "' and user_vali='2'";
        ...
        private void modifyAllowLogin()
        {
            ...
            if (res == 0) { MessageBox.Show("修改用户权限失败，请重试！", "友情提示"); return; }
